...
inline void update_motion_pick_above(
    ...
    KDL::JntArrayVel q_qd_arm_solver_pick_above(state.arm_solver_pick_above.q, state.arm_solver_pick_above.qd);
    {
        ...
        KDL::Frame _base_world_frame_arm_solver_pick_above;
        mj_kdl::get_body_frame(
                robot.arm_solver_pick_above.robot->model,
                robot.arm_solver_pick_above.robot->data,
                "base_link",
                &_base_world_frame_arm_solver_pick_above);
        shared.pose_cube_base = _base_world_frame_arm_solver_pick_above.Inverse() * _body_frame_pose_cube_base;
    }
...
inline bool can_start_motion_pick_above(
    ...
    // eval_pick_above_when_gripper_ready
    shared.eval_pick_above_when_gripper_ready_err = motion_spec::runtime::evaluate_equality_constraint(shared.gripper_open, shared.gripper_pos);

    return motion_spec::runtime::constraint_satisfied(shared.eval_pick_above_when_gripper_ready_err, shared.satisfied_band_rot);
}
...
inline void monitor_when_motion_pick_above(
    ...
    {
        const bool active = motion_spec::runtime::constraint_satisfied(shared.eval_pick_above_when_gripper_ready_err, shared.satisfied_band_rot);
        ...
        if (detected && robot.fsm_events) {
            produce_event(robot.fsm_events, pick_place_single_fsm::E_PICK_ABOVE_READY);
            std::cerr << "[fsm] event   " << pick_place_single_fsm::EVENT_URIS[pick_place_single_fsm::E_PICK_ABOVE_READY] << std::endl;
        }
    }
