...
inline void update_motion_place_above(
    ...
    KDL::JntArrayVel q_qd_arm_solver_place_above(state.arm_solver_place_above.q, state.arm_solver_place_above.qd);
    {
        ...
        KDL::Frame _base_world_frame_arm_solver_place_above;
        mj_kdl::get_body_frame(
                robot.arm_solver_place_above.robot->model,
                robot.arm_solver_place_above.robot->data,
                "base_link",
                &_base_world_frame_arm_solver_place_above);
        shared.pose_cube_base = _base_world_frame_arm_solver_place_above.Inverse() * _body_frame_pose_cube_base;
    }
...
inline void monitor_when_motion_place_above(
...

    {
        ...
        if (detected && robot.fsm_events) {
            produce_event(robot.fsm_events, pick_place_single_fsm::E_PLACE_ABOVE_READY);
            std::cerr << "[fsm] event   " << pick_place_single_fsm::EVENT_URIS[pick_place_single_fsm::E_PLACE_ABOVE_READY] << std::endl;
        }
    }
...
inline void monitor_until_motion_place_above(
    ...
    shared.eval_place_above_until_grasp_lost_err = motion_spec::runtime::evaluate_greater_than_constraint(shared.distance_grasp_lost_distance, shared.lost_dist);

    {
        const bool active = motion_spec::runtime::constraint_satisfied(shared.eval_place_above_until_grasp_lost_err, shared.default_tolerance_Distance);
        ...
        if (detected && robot.fsm_events) {
            produce_event(robot.fsm_events, pick_place_single_fsm::E_GRASP_LOST_PLACE_ABOVE);
            std::cerr << "[fsm] event   " << pick_place_single_fsm::EVENT_URIS[pick_place_single_fsm::E_GRASP_LOST_PLACE_ABOVE] << std::endl;
        }
    }
