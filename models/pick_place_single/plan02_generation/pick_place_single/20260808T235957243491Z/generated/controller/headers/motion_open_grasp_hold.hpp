...
inline void update_motion_open_grasp_hold(
    ...
    KDL::JntArrayVel q_qd_arm_solver_open_grasp_hold(state.arm_solver_open_grasp_hold.q, state.arm_solver_open_grasp_hold.qd);
    {
        ...
        KDL::Frame _base_world_frame_arm_solver_open_grasp_hold;
        mj_kdl::get_body_frame(
                robot.arm_solver_open_grasp_hold.robot->model,
                robot.arm_solver_open_grasp_hold.robot->data,
                "base_link",
                &_base_world_frame_arm_solver_open_grasp_hold);
        shared.pose_cube_base = _base_world_frame_arm_solver_open_grasp_hold.Inverse() * _body_frame_pose_cube_base;
    }
...
inline void monitor_when_motion_open_grasp_hold(
...

    {
        ...
        if (detected && robot.fsm_events) {
            produce_event(robot.fsm_events, pick_place_single_fsm::E_OPEN_READY);
            std::cerr << "[fsm] event   " << pick_place_single_fsm::EVENT_URIS[pick_place_single_fsm::E_OPEN_READY] << std::endl;
        }
    }
