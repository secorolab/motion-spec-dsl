...
inline void update_motion_lift(
    ...
    KDL::JntArrayVel q_qd_arm_solver_lift(state.arm_solver_lift.q, state.arm_solver_lift.qd);
    {
        ...
        KDL::Frame _base_world_frame_arm_solver_lift;
        mj_kdl::get_body_frame(
                robot.arm_solver_lift.robot->model,
                robot.arm_solver_lift.robot->data,
                "base_link",
                &_base_world_frame_arm_solver_lift);
        shared.pose_cube_base = _base_world_frame_arm_solver_lift.Inverse() * _body_frame_pose_cube_base;
    }
...
inline void monitor_when_motion_lift(
...

    {
        ...
        if (detected && robot.fsm_events) {
            produce_event(robot.fsm_events, pick_place_single_fsm::E_LIFT_READY);
            std::cerr << "[fsm] event   " << pick_place_single_fsm::EVENT_URIS[pick_place_single_fsm::E_LIFT_READY] << std::endl;
        }
    }
...
inline void monitor_until_motion_lift(
...

    {
        ...
        if (detected && robot.fsm_events) {
            produce_event(robot.fsm_events, pick_place_single_fsm::E_GRASP_LOST_LIFT);
            std::cerr << "[fsm] event   " << pick_place_single_fsm::EVENT_URIS[pick_place_single_fsm::E_GRASP_LOST_LIFT] << std::endl;
        }
    }
