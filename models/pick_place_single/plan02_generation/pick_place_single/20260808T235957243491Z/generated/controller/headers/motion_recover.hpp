...
inline void update_motion_recover(
    ...
    KDL::JntArrayVel q_qd_arm_solver_recover(state.arm_solver_recover.q, state.arm_solver_recover.qd);
    {
        ...
        KDL::Frame _base_world_frame_arm_solver_recover;
        mj_kdl::get_body_frame(
                robot.arm_solver_recover.robot->model,
                robot.arm_solver_recover.robot->data,
                "base_link",
                &_base_world_frame_arm_solver_recover);
        shared.pose_cube_base = _base_world_frame_arm_solver_recover.Inverse() * _body_frame_pose_cube_base;
    }
...
inline void control_motion_recover(
...

    {
        ...
        if (detected && robot.fsm_events) {
            produce_event(robot.fsm_events, pick_place_single_fsm::E_RECOVER);
            std::cerr << "[fsm] event   " << pick_place_single_fsm::EVENT_URIS[pick_place_single_fsm::E_RECOVER] << std::endl;
        }
    }
