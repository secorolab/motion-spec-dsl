...
inline void update_motion_pick(
    ...
    KDL::JntArrayVel q_qd_arm_solver_pick(state.arm_solver_pick.q, state.arm_solver_pick.qd);
    {
        ...
        KDL::Frame _base_world_frame_arm_solver_pick;
        mj_kdl::get_body_frame(
                robot.arm_solver_pick.robot->model,
                robot.arm_solver_pick.robot->data,
                "base_link",
                &_base_world_frame_arm_solver_pick);
        shared.pose_cube_base = _base_world_frame_arm_solver_pick.Inverse() * _body_frame_pose_cube_base;
    }
...
inline void monitor_when_motion_pick(
...

    {
        ...
        if (detected && robot.fsm_events) {
            produce_event(robot.fsm_events, pick_place_single_fsm::E_PICK_READY);
            std::cerr << "[fsm] event   " << pick_place_single_fsm::EVENT_URIS[pick_place_single_fsm::E_PICK_READY] << std::endl;
        }
    }
