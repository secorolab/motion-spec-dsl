...
struct motion_home_state {
    ...
    bool mon_home_settled_event_triggered = false;
    double mon_home_settled_hold_s = 0.0;
...
inline void update_motion_home(
    ...
    KDL::JntArrayVel q_qd_arm_solver_home(state.arm_solver_home.q, state.arm_solver_home.qd);
    {
        ...
        fk.JntToCart(
            ...
            motion_spec::runtime::find_segment_index(*robot.arm_solver_home.chain, "bracelet_link", "base_link"));
    }
    ...
    {
        const mj_kdl::ForceTorqueSensor *ft_ext_force = mj_kdl::find_ft_sensor(robot.arm_solver_home.robot, "wrist_ft");
        if (!ft_ext_force) {
            throw std::runtime_error("FT sensor 'wrist_ft' is unavailable");
        }
        const auto frame_ext_force = [&](const char *name) {
            KDL::Frame frame;
            if (!mj_kdl::get_site_frame(robot.arm_solver_home.robot->model, robot.arm_solver_home.robot->data, name, &frame)
                && !mj_kdl::get_body_frame(robot.arm_solver_home.robot->model, robot.arm_solver_home.robot->data, name, &frame)) {
...
inline void monitor_until_motion_home(
    ...
    // eval_home_until_settled_x
    shared.eval_home_until_settled_x_err = motion_spec::runtime::evaluate_equality_constraint(shared.zero_linvel, shared.twist_ee_base.vel[0]);
    // eval_home_until_settled_y
    shared.eval_home_until_settled_y_err = motion_spec::runtime::evaluate_equality_constraint(shared.zero_linvel, shared.twist_ee_base.vel[1]);
    // eval_home_until_settled_z
    shared.eval_home_until_settled_z_err = motion_spec::runtime::evaluate_equality_constraint(shared.zero_linvel, shared.twist_ee_base.vel[2]);

    {
        const bool active = (motion_spec::runtime::constraint_satisfied(shared.eval_home_until_settled_x_err, shared.satisfied_band_vel) && motion_spec::runtime::constraint_satisfied(shared.eval_home_until_settled_y_err, shared.satisfied_band_vel) && motion_spec::runtime::constraint_satisfied(shared.eval_home_until_settled_z_err, shared.satisfied_band_vel));
        const bool detected = motion_spec::runtime::sustained_edge(
            state.mon_home_settled_hold_s,
            active,
        ...
        if (detected && robot.fsm_events) {
            produce_event(robot.fsm_events, admittance_arc_single_fsm::E_HOME_SETTLED);
            std::cerr << "[fsm] event   " << admittance_arc_single_fsm::EVENT_URIS[admittance_arc_single_fsm::E_HOME_SETTLED] << std::endl;
        }
    }
