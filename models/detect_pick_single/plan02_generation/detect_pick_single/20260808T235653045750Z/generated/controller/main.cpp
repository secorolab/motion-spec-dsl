...
int main(int argc, char **argv) {
    ...
    while (!motion_spec::runtime::stop_requested() && ((!headless || !steps_limited || headless_steps-- > 0)) && rclcpp::ok()) {
        ...
        if (_bhv_goal && !_bhv_goal_armed) {
            ...
            produce_event(robot.fsm_events, 3);
        }
        if (_bhv_goal && consume_event(robot.fsm_events, 0)) {
            robot.bhv_event_msg.scenario_context_id.uuid = _scenario_context_id;
            robot.bhv_event_msg.stamp = robot.ros_node->now();
            robot.bhv_event_msg.uri = detect_pick_single_fsm::EVENT_URIS[0];
            robot.bhv_events_pub->try_publish(robot.bhv_event_msg);
        }
        if (_bhv_goal && consume_event(robot.fsm_events, 7)) {
            robot.bhv_event_msg.scenario_context_id.uuid = _scenario_context_id;
            robot.bhv_event_msg.stamp = robot.ros_node->now();
            robot.bhv_event_msg.uri = detect_pick_single_fsm::EVENT_URIS[7];
            robot.bhv_events_pub->try_publish(robot.bhv_event_msg);
        }
        if (_bhv_goal && consume_event(robot.fsm_events, 15)) {
            robot.bhv_event_msg.scenario_context_id.uuid = _scenario_context_id;
            robot.bhv_event_msg.stamp = robot.ros_node->now();
            robot.bhv_event_msg.uri = detect_pick_single_fsm::EVENT_URIS[15];
            robot.bhv_events_pub->try_publish(robot.bhv_event_msg);
        }
