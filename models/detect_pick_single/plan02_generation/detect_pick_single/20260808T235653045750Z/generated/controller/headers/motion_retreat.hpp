...
inline void monitor_when_motion_retreat(
...

    {
        ...
        if (detected && robot.fsm_events) {
            produce_event(robot.fsm_events, detect_pick_single_fsm::E_RETREAT_READY);
            std::cerr << "[fsm] event   " << detect_pick_single_fsm::EVENT_URIS[detect_pick_single_fsm::E_RETREAT_READY] << std::endl;
        }
    }
...
inline void monitor_until_motion_retreat(
...

    {
        ...
        if (detected && robot.fsm_events) {
            produce_event(robot.fsm_events, detect_pick_single_fsm::E_RETREAT_SETTLED);
            std::cerr << "[fsm] event   " << detect_pick_single_fsm::EVENT_URIS[detect_pick_single_fsm::E_RETREAT_SETTLED] << std::endl;
        }
    }
