...
8. Fixed: FeedForward/gripper equality constraints now emit assignment evaluators, satisfying handler evaluator coverage.
...
15. Fixed: pose-valued trajectory nodes are typed as `geom-coord:PoseCoordinate` when their lerp endpoints are poses.
16. Fixed: emitted PoseCoordinate metadata now follows comp-rob2b `as-seen-by == with-respect-to` for measured, declared, and trajectory pose coordinates.
